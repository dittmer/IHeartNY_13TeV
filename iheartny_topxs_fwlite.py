...
if options.fullTruth:
    ...
    trueTree.Branch('partAK4jetPt'           , partAK4jetPt           )
    trueTree.Branch('partAK4jetEta'          , partAK4jetEta          )
    trueTree.Branch('partAK4jetPhi'          , partAK4jetPhi          )
    trueTree.Branch('partAK4jetMass'         , partAK4jetMass         )
    trueTree.Branch('partAK8jetPt'           , partAK8jetPt           )
    trueTree.Branch('partAK8jetEta'          , partAK8jetEta          )
    trueTree.Branch('partAK8jetPhi'          , partAK8jetPhi          )
    trueTree.Branch('partAK8jetMass'         , partAK8jetMass         )
    trueTree.Branch('partMuPt'               , partMuPt               )
    trueTree.Branch('partMuEta'              , partMuEta              )
    trueTree.Branch('partMuPhi'              , partMuPhi              )
    trueTree.Branch('partMuMass'             , partMuMass             )
    trueTree.Branch('partElPt'               , partElPt               )
    trueTree.Branch('partElEta'              , partElEta              )
    trueTree.Branch('partElPhi'              , partElPhi              )
    trueTree.Branch('partElMass'             , partElMass             )
    ...
    if electronPtHandle.isValid() :
        ...
        for ielectronPt in range(0,len(electronPts)) :
            ...
            if manualEisMedium and passD0 and passDz :
                nTrueMedium += 1.0
            if manualEisMediumNoVeto and passD0 and passDz and isElectronConVeto[ielectronPt] :
                nFakeMedium += 1.0
            if manualEisTight and passD0 and passDz :
                nTrueTight += 1.0
            if manualEisTightNoVeto and passD0 and passDz and isElectronConVeto[ielectronPt] :
                nFakeTight += 1.0

            if not (manualEisMediumNoVeto and passD0 and passDz) :
                continue
            ...
            if manualEisMedium : #Electrons for cleaning pass full Medium ID
                elCand.append(p4)
                lepCand.append(p4)
                elCandKey.append(elKeys[ielectronPt])
                lepCandKey.append(elKeys[ielectronPt])
                
            if manualEisTightNoVeto :
                elTight.push_back(1)
            else :
                elTight.push_back(0)
...
print '  mu channel:           ' + str(nPassLepMu)
